...
$pornombre_status = 'unchecked';
$porautor_status = 'unchecked';

if (isset($_POST['btn'])) {
$selected_radio = $_POST['gender'];
if ($selected_radio == 'pornombre') {
$pornombre_status = 'checked';
}
else if ($selected_radio == 'porautor') {
$porautor_status = 'checked';
}
}
                             ...
                             <div class="col-md-3">
                                <div class="search-box">  
                                    <form name="search_form" method="post" class="search_form" action="buscador.php">
                                        ...
                                        <input  class="radio" type = 'Radio' Name ='gender' value= 'pornombre' <?php print $pornombre_status; ?>><label class="e-radio">Por nombre</label>
                                       <input  class="radio" type = 'Radio' Name ='gender' value= 'porautor' <?php print $porautor_status; ?>><label class="e-radio">Por autor</label>
...
$buscarpornombre=$_POST['gender'];
if ($buscarpornombre == 'pornombre') 
{
//buscar por nombre del libro
               $titulo = $_POST['titulo'];
                $query = mysql_query("SELECT * FROM libros where NombreLibro like'%".$titulo."%'", $conexion);
...
{
$buscarporautor=$_POST['gender'];

if ($buscarporautor == 'porautor') { 
//buscar por nombre del autor del libro
                    $autorlibro = $_POST['titulo'];
                    $query = mysql_query("SELECT * FROM libros where AutorLibro like'%".$autorlibro."%'", $conexion);
                    while ($fila = mysql_fetch_array($query)) {
                    $id = $fila['IdLibro'];
                    $nombrelibro = $fila['NombreLibro'];
                    $autor =$fila['AutorLibro'];
